...
# print(output)
#filter good quality
command = "%s/bcftools filter -i'QUAL>%s && FMT/DP>%s' %s.variants.vcf > %s.filtered.genome.q%s.dp%s.vcf" % (bcftools_path, quality, depth, base_name, base_name, quality, depth)
output = call(command, shell=True)
print(output)

#clean_nonref
vcf_reader = open("%s.filtered.genome.q%s.dp%s.vcf" % (base_name, quality, depth))
...
command = 'oncotator -v --db-dir %s -i VCF %s.oncotator.vcf %s.q%s.dp%s.oncotator.tsv hg19' % (oncotator_path, base_name, base_name, quality, depth)
output = call(command, shell=True)
print(output)
